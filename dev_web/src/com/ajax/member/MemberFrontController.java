...
public class MemberFrontController extends HttpServlet {
	...
	throws ServletException, IOException
	{
		...
		if("/intro/login.mem".equals(command)){//로그인 버튼을 클릭했을 때
			...
			forward = action.execute(req, res);
		}else if("/intro/getMemberList.mem".equals(command)){//로그인 버튼을 클릭했을 때
			logger.info("회원목록버튼을 눌렀을 때.");
		...
		else if("/intro/getReceiveMemoList.mem".equals(command)){//로그인 버튼을 클릭했을 때
			logger.info("받은 쪽지함.");
		...
		else if("/intro/memoDelete.mem".equals(command)){//로그인 버튼을 클릭했을 때
			logger.info("쪽지 삭제.");
			action = new MemoDeleteAction();
			forward = action.execute(req, res);
		}	
		else if("/intro/memoUpate.mem".equals(command)){//로그인 버튼을 클릭했을 때
			logger.info("쪽지 수정.");
